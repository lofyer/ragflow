...
async def graph_node_to_chunk(kb_id, embd_mdl, ent_name, meta, chunks):
    ...
    if ebd is None:
        async with chat_limiter:
            ...
            try:
                logging.info(f"[GraphRAG] Embedding node: {ent_name[:50]}... model={embd_mdl.llm_name}")
                ebd, _ = await asyncio.wait_for(
                    asyncio.to_thread(embd_mdl.encode, [ent_name]),
                    timeout=timeout_val
                )
                logging.info(f"[GraphRAG] Embedding node success: {ent_name[:50]}...")
            except Exception as e:
                error_msg = str(e)
                error_type = type(e).__name__
                logging.error(f"[GraphRAG] Embedding node FAILED: {ent_name[:50]}... error_type={error_type}, error={error_msg}")
                if hasattr(e, 'response'):
                    logging.error(f"[GraphRAG] Response details: {getattr(e, 'response', None)}")
                if hasattr(e, 'status_code'):
                    logging.error(f"[GraphRAG] Status code: {getattr(e, 'status_code', None)}")
                if hasattr(e, 'code'):
                    logging.error(f"[GraphRAG] Error code: {getattr(e, 'code', None)}")
                raise
        ebd = ebd[0]
...
async def graph_edge_to_chunk(kb_id, embd_mdl, from_ent_name, to_ent_name, meta, chunks):
    ...
    if ebd is None:
        async with chat_limiter:
            timeout_val = 3 if enable_timeout_assertion else 300000000
            try:
                logging.info(f"[GraphRAG] Embedding edge: {txt[:50]}... model={embd_mdl.llm_name}")
                ebd, _ = await asyncio.wait_for(
                    asyncio.to_thread(
                        embd_mdl.encode,
                ...
                logging.info(f"[GraphRAG] Embedding edge success: {txt[:50]}...")
            except Exception as e:
                error_msg = str(e)
                error_type = type(e).__name__
                logging.error(f"[GraphRAG] Embedding edge FAILED: {txt[:50]}... error_type={error_type}, error={error_msg}")
                if hasattr(e, 'response'):
                    logging.error(f"[GraphRAG] Response details: {getattr(e, 'response', None)}")
                if hasattr(e, 'status_code'):
                    logging.error(f"[GraphRAG] Status code: {getattr(e, 'status_code', None)}")
