...
if [[ "${ENABLE_WEBSERVER}" -eq 1 ]]; then
    ...
    "$PY" -c "import gunicorn" 2>/dev/null || uv pip install gunicorn -i https://pypi.tuna.tsinghua.edu.cn/simple

    # Initialize database
    echo "Initializing database..."
    "$PY" api/ragflow_server.py --version || true

    echo "Starting ragflow_server with gunicorn..."
    GUNICORN_WORKERS=${GUNICORN_WORKERS:-16}
    while true; do
        "$PY" -m gunicorn -w ${GUNICORN_WORKERS} \
                 -b 0.0.0.0:9380 \
                 --timeout 300 \
                 --keep-alive 5 \
                 api.apps:app
